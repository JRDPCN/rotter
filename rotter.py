...
{Fore.BLUE}||{Style.RESET_ALL} {Fore.CYAN} by JRDP Team  https://github.com/JRDPCN  {Style.RESET_ALL}         {Fore.BLUE}||{Style.RESET_ALL}
...
def main():
    ...
    if cipher_index is not None and encrypt_index is not None and decrypt_index is None:
    ...
    if cipher_index is not None and decrypt_index is not None and encrypt_index is None:
        cipher = args[cipher_index + 1]
        shift = int(cipher[3:]) if cipher.startswith('ROT') and cipher[3:].isdigit() else None
        if shift and 1 <= shift <= 100:
